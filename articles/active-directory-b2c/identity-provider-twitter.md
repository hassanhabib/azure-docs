---
title: Set up sign-up and sign-in with a X account
...
#Customer Intent: As a developer setting up sign-up and sign-in with a X account using Azure Active Directory B2C, I want to configure X as an identity provider so that I can enable users to sign in with their X accounts.

---

# Set up sign-up and sign-in with a X account using Azure Active Directory B2C
...
1. Select **Identity providers**, then select **X**.
...
1. Under the **Social identity providers**, select **X**.
1. Select **Save**.
...
1. From the sign-up or sign-in page, select **X** to sign in with X account.
...
To enable users to sign in using a X account, you need to define the account as a claims provider that Azure AD B2C can communicate with through an endpoint. The endpoint provides a set of claims that are used by Azure AD B2C to verify that a specific user has authenticated.

You can define a X account as a claims provider by adding it to the **ClaimsProviders** element in the extension file of your policy. Refer to the custom policy starter pack that you downloaded in the Prerequisites of this article.
...
3. Add a new **ClaimsProvider** as follows:
    ...
    <ClaimsProvider>
      ...
      <TechnicalProfiles>
        <TechnicalProfile Id="X-OAuth1">
          <DisplayName>X</DisplayName>
          <Protocol Name="OAuth1" />
          <Metadata>
            <Item Key="ProviderName">X</Item>
            <Item Key="authorization_endpoint">https://api.X.com/oauth/authenticate</Item>
            <Item Key="access_token_endpoint">https://api.X.com/oauth/access_token</Item>
            <Item Key="request_token_endpoint">https://api.X.com/oauth/request_token</Item>
            <Item Key="ClaimsEndpoint">https://api.X.com/1.1/account/verify_credentials.json?include_email=true</Item>
            <Item Key="ClaimsResponseFormat">json</Item>
            <Item Key="client_id">Your X application API key</Item>
          </Metadata>
          <CryptographicKeys>
            <Key Id="client_secret" StorageReferenceId="B2C_1A_XSecret" />
          </CryptographicKeys>
          <OutputClaims>
            <OutputClaim ClaimTypeReferenceId="issuerUserId" PartnerClaimType="user_id" />
            <OutputClaim ClaimTypeReferenceId="displayName" PartnerClaimType="screen_name" />
            <OutputClaim ClaimTypeReferenceId="email" />
            <OutputClaim ClaimTypeReferenceId="identityProvider" DefaultValue="X.com" />
...
<OrchestrationStep Order="1" Type="CombinedSignInAndSignUp" ContentDefinitionReferenceId="api.signuporsignin">
  <ClaimsProviderSelections>
    ...
    <ClaimsProviderSelection TargetClaimsExchangeId="XExchange" />
  </ClaimsProviderSelections>
  ...
</OrchestrationStep>
...
<OrchestrationStep Order="2" Type="ClaimsExchange">
  ...
  <ClaimsExchanges>
    <ClaimsExchange Id="XExchange" TechnicalProfileReferenceId="X-OAuth1" />
  </ClaimsExchanges>
</OrchestrationStep>
```
